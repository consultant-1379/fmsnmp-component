...
public class HandleAlarmSeverity {
	/**
	 * to avoid PMD Warnings
	 */
	private HandleAlarmSeverity(){
		
	}
	private static final Logger LOGGER = LoggerFactory
			.getLogger(HandleAlarmSeverity.class);

	// AXD axdperceivedSeverity
	public static String getaxdAlarmNumber(final String axdperceivedSeverity,
			final String attributename, final String attributevalue) {
		String axdAlarmNum = "";
		try {

			final int sevOIDLen = HandleOtherAlarmAttribute.AXDSEV.length();
			System.out.println("SEV OID len >> " + sevOIDLen);

			final String axdSevToken = axdperceivedSeverity.substring(sevOIDLen);
			System.out.println("axdSevToken >> " + axdSevToken);
			axdAlarmNum = axdSevToken.substring(
					axdSevToken.lastIndexOf(".") + 1, axdSevToken.length());
			System.out.println(" axd301FaultIdSev >> " + axdAlarmNum);

		} catch (Exception e) {
			System.out.println("Exception in setting ExternalEventId >> "
					+ e.toString());
			return axdAlarmNum;
		}
		return axdAlarmNum;
	...
	public static String getPerceivedSeverity(final String severity) {
		...
		try {
			...
			System.out.println(" Exception in getPerceivedSeverity ()--> "
					+ e.toString());
			perceivedSeverity = Constants.SEV_INDETERMINATE;
		}
	...
	public static String getTigrisSeverity(final String attributeValue) {
		...
		try {
			...
			System.out.println("setPerceivedSeverity Exception." + ex.getMessage());
			tigrisseverity = Constants.SEV_INDETERMINATE;
	...
	public static String getIntegerPerceivedSeverity(int severity) 
	{ 
		String perceivedSeverity= Constants.SEV_INDETERMINATE;; 
		int severityIntValue = severity; 
		try 
		{ 
			switch(severityIntValue) 
			{ 
		...
		{ 
			LOGGER.error("Exception in getPerceivedSeverity ",e); 
			perceivedSeverity =Constants.SEV_INDETERMINATE;
		} 
		return perceivedSeverity; 
	}
